...
class Sass::Tree::Visitors::Convert < Sass::Tree::Visitors::Base
  ...
  def visit_children(parent)
    ...
    (@format == :styles ? "\n" : " {\n") + super.join.rstrip + (@format == :styles ? "\n" : "\n#{ @tab_chars * (@tabs-1)}}\n")
  ensure
    @tabs -= 1
  end
  ...
  def visit_mixindef(node)
    ...
    "#{tab_str}#{@format == :styles ? '=' : '@mixin '}#{dasherize(node.name)}#{args}#{yield}"
  end
  ...
  def visit_mixin(node)
    ...
    "#{tab_str}#{@format == :styles ? '+' : '@include '}#{dasherize(node.name)}#{arglist}#{node.has_children ? yield : semi}\n"
  end
  ...
  def selector_to_src(sel)
    @format == :styles ? selector_to_sass(sel) : selector_to_scss(sel)
  end
